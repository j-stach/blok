...
impl<B: Block> Layer<B> {
    ...
    pub fn populate(&mut self, mut layout: Layout, c: B::Constructor, i: &B::ConstructionInstructions) -> &mut Self {
        for row in layout.iter() {
            for _ in 0..*row {
                self.blocks_mut().push(c(i))
            }
        }
    ...
    pub fn fill_voids(&mut self, c: &B::Constructor, i: &B::ConstructionInstructions) {
